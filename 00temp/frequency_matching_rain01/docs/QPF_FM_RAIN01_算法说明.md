...
- 与默认 MICAPS 接口不同的全新数据格式（需在 `src/data/types.py` 扩展读写）。
...
| 路径 | 作用 |
|------|------|
| `cli/` | `python -m cli` 转调 `src/runner.py` |
| `src/runner.py` | 主程序：`main()`、配置解析与订正流水线 |
| `src/data/types.py` | `GridData`、`ScatterData`、MICAPS/二进制 I/O |
...
| `utils/util_env.py` | 从 `resource/qpf_fm.ini` 解析路径 |
| `utils/util_paths.py` | 仓库根目录与相对路径展开 |
| `utils/string_process.py` | 路径模板 `date_replace`（`YYYY`、`VVV` 等） |
...
| 路径模板替换 | `utils/string_process.py` | `date_replace()` |
...
python -m cli --data=grapes_3km 202604081130    # 等价 --path=
```

| 参数形式 | 说明 |
|----------|------|
| 第 1 个非时间 token | 模式键（对应 `path.json` → `configs`） |
| `YYYYMMDDHHMM` × 1 | 单个运行时刻 |
| `YYYYMMDDHHMM` × 2 | 起止时刻，闭区间，步长 1 小时 |
...
python src/runner.py ecmwf 202604081130 202604081800
```
...
| 测试文件 | 覆盖内容 |
|----------|----------|
| `test_qpf_fm_rain01.py` | 频率匹配、`date_replace` |
...
在相同输入数据与配置下，订正算法路径与数值行为应与原版一致；重构仅涉及目录、配置与模块划分，不改变 `proc/` 与 `data/` 中的计算逻辑。
