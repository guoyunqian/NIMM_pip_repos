...
def _compute_inverse_acf_mapping(target_dist, target_dist_params, n_intervals=10):
    ...
    def cdf_trans(x): return target_dist.ppf(stats.norm.cdf(x), *target_dist_params)
    int_range = (-6, 6)
...
def _fit_dist(err, dist, wf, mask):
    ...
    def func(p): return -np.sum(np.log(stats.lognorm.pdf(err[mask], p, -0.5 * p**2)))
...
def _linda_deterministic_init(
    ...
    from .utils import feature_get_method
    ...
    extrapolator = extrapolation_get_method(extrap_method)
    extrap_kwargs = extrap_kwargs.copy()
    ...
    if feature_method in {"blob", "shitomasi"}:
        ...
        feature_detector = feature_get_method(feature_method)
