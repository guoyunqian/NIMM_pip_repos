...
def do_gs_merge(
    ...
    # step1: bias = 站 − 格点插值到站
    g2s = meb.interp_gs_linear(ifcst_g, sta)
    ...
    if terr_mat is not None:
        hgt_s = meb.interp_gs_linear(terr_mat, sta)
    ...
    for i in range(len(slons)):
        ...
        if _terr is not None:
            istat_dhgt = _terr[idx_lm_s[i]:idx_lm_n[i] + 1, idx_lm_w[i]:idx_lm_e[i] + 1] - shgts[i]
            mask = (np.abs(istat_dhgt) < 100).astype(np.int32)
            _iw = w_basic[idx_sm_s[i]:idx_sm_n[i] + 1, idx_sm_w[i]:idx_sm_e[i] + 1] * mask
        else:
            _iw = w_basic[idx_sm_s[i]:idx_sm_n[i] + 1, idx_sm_w[i]:idx_sm_e[i] + 1]

        bias_mat[idx_lm_s[i]:idx_lm_n[i] + 1, idx_lm_w[i]:idx_lm_e[i] + 1] += bias[i] * _iw
        w_mat[idx_lm_s[i]:idx_lm_n[i] + 1, idx_lm_w[i]:idx_lm_e[i] + 1] += _iw
