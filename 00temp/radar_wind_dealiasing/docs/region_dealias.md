...
```python
from radar_wind_dealiasing.cli.polar_volume_main import process

velocity = process(
    "radar_wind_dealiasing/test_data/region_dealias/cli_input/radar_fixed.cfradial.nc",
    "velocity",
    output_path="radar_wind_dealiasing/test_data/region_dealias/cli_input/velocity_volume.nc",
    require_geolocation=True,
)
```

该工具是 CLI 层的数据预处理入口，不属于核心退模糊算法。
脚本 `__main__` 与验证 notebook 对齐：优先复用固定 CfRadial，再转换并写出
`cli_input/velocity_volume.nc`。验证 notebook 当前只落盘速度体扫与 gatefilter 掩码；
...
mask = np.load(
    ...
    "grid_gatefilter_mask_volume.npy"
).astype(bool)
...
process(
    "radar_wind_dealiasing/test_data/region_dealias/cli_input/velocity_volume.nc",
    gatefilter_path=(
        "radar_wind_dealiasing/test_data/region_dealias/cli_input/"
        "grid_gatefilter_mask_volume.npy"
    ),
...
示例 `__main__` 默认读取 `cli_input/velocity_volume.nc` 与
`cli_input/grid_gatefilter_mask_volume.npy`。
...
|---|---|
|`095636.mdv`|官方示例源数据。|
|`cli_input/radar_fixed.cfradial.nc`|固定化 CfRadial，便于复现。|
|`cli_input/velocity_volume.nc`|极坐标速度体扫输入。|
|`cli_input/grid_gatefilter_mask_volume.npy`|完整体扫过滤掩码。|
|`cli_output/*.nc`|CLI 输出结果。|
