# -*- coding: utf-8 -*-
"""从 ``resource/interp_gg_linear.ini`` 读取运行参数与路径。"""
...
_MAIN_INI_REL = os.path.join("resource", "interp_gg_linear.ini")
...
def get_resolved_paths() -> Dict[str, str]:
    return {
        ...
        "output_path": _expand_path_maybe_relative_to_repo(
            _ini_get("output_path", "resource/output/gg_linear.m4")),
    }


def get_linear_params() -> Dict:
    glon = _ini_float_list("glon", [])
    glat = _ini_float_list("glat", [])
    domain = _ini_float_list("domain", [])
    outer = _ini_get("outer_value")
    used = (_ini_get("used_coords", "xy") or "xy").strip().lower()
    return {
        "used_coords": used if used else "xy",
