# -*- coding: utf-8 -*-
"""
1 小时气象预报集成 CLI（``python -m cli`` → ``mait_1h.process``）。
...
::
    ...
    python -m cli --time-inputs=202401010800 --predict-valid-list=1,2,3
    python -m cli verify ts --h5-file=... --grade-list=... --product-list=...

模块调用::

    from mait_1h import process
...
def _make_cli_entry():
    ...
    def run_cli(
        *,
        ...
        :param predict_valid_list: 预报时效列表（小时），逗号分隔，如 1,2,3；省略则读 resource/mait_1.ini
        :param para_path: 模式/实况路径参数文件 local.ini / para_1.ini 路径；省略则用 ini 中的 para_ini
        :param beta_path: beta 权重文件路径模板（可含 YYYYMMDDHH）；省略则用 ini 中的 beta_path_template
        ...
        :param pro_count: 多进程并行进程数；省略则读 mait_1.ini 的 pro_count
        :param split_lat: 纬度方向分块数（站点插值分块）；省略则读 mait_1.ini
        :param split_lon: 经度方向分块数（站点插值分块）；省略则读 mait_1.ini
        """
        process(
