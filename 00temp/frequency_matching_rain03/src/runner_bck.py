...
def _run_parallel_lead_time_tasks(script_path, cwd, argv, tasks, text2, text4, log):
    ...
    for date_time, predict_valid in tasks:
        ...
        path2 = StringProcess.date_replace(text4 + ".m3", date_time, predict_valid)
        path3 = StringProcess.date_replace(text4 + ".m4", date_time, predict_valid)
        path4 = StringProcess.date_replace(text2, date_time, predict_valid)
        print("=-------------------------->>>> desc: ", desc)
        print("=-------------------------->>>> path2: ", path2)
        print("=-------------------------->>>> path3: ", path3)
        print("=-------------------------->>>> path4: ", path4)

        if (os.path.exists(path2) and os.path.exists(path3)) or not os.path.exists(path4):
            if os.path.exists(path2) and os.path.exists(path3):
                log.write_info(f"[SKIP] {desc}: output files already exist", 0)
            else:
                log.write_info(f"[SKIP] {desc}: model input not found: {path4}", 0)
            summary["skip"] += 1
            continue
...
def main(argv=None):
    ...
    log = Log(StringProcess.date_replace(log_tpl, datetime.now(), 0))
            ...
            try:
                ...
                if (os.path.exists(path2) and os.path.exists(path3)) or not os.path.exists(path4):
                    if os.path.exists(path2) and os.path.exists(path3):
                        ...
                        log.write_info(f"[SKIP] {task_desc}: model input not found: {path4}", 0)
                    skip_tasks += 1
                    continue
                ...
                dt_input = date_time
                text6 = StringProcess.date_replace(text2, dt_input, predict_valid)
                ...
                for j in range(num7):
                    ...
                    while date_time4 <= date_time3:
                        ...
                        text7 = StringProcess.date_replace(text2, date_time4, predict_valid)
                        text8 = StringProcess.date_replace(text3, dt_input2, 0)

                        if os.path.exists(text7) and os.path.exists(text8):
                            ...
                            missing_files = []
                            if not os.path.exists(text7):
                                missing_files.append(f"model={text7}")
                                hist_missing += 1
                            if not os.path.exists(text8):
                                missing_files.append(f"fact={text8}")
                                hist_missing += 1
                            if missing_files:
                                log.write_info(
                            ...
                            if date_time4 == date_time2 or date_time4 == date_time3:
                                print("lack dtModel: " + text7)
                                print("lack dtFact: " + text8)
                ...
                str_header = StringProcess.date_replace(
                    ...
                    date_time, predict_valid)
                text9 = StringProcess.date_replace(text4 + ".m3", date_time, predict_valid)
                print(text9)
                ...
                str_header2 = StringProcess.date_replace(
                    ...
                    date_time, predict_valid)
                text10 = StringProcess.date_replace(text4, date_time, predict_valid)

                # .cybin 和 .m4raw 输出暂时关闭
                # grid_data3.write_val_to_cybin(text10 + ".cybin")
                try:
                    grid_data3.write_val_to_micaps4(text10 + ".m4", str_header2)
