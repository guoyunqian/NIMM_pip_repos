...
"""将官方投影样例预处理为 meb，并可选写出经纬重网格对照输入。

两条路径
--------
1. **投影输入**  
   ``源目录/cli_input/<同名>.nc``：投影米制坐标仅改名为 ``lat``/``lon`` → meb 六维。

2. **经纬输入**  
   - ``源目录/latlon/<同名>.nc``：经纬重网格后的 **Iris Cube**（供原 IMPROVER 方法）  
   - ``源目录/latlon/cli_input/<同名>.nc``：同一数值的 **meb 六维**（供迁移方法）  
   两边仅格式不同，场值一致。

3. **经纬 KGO**  
   各场景投影 ``kgo.nc`` 重网格为 ``latlon/kgo.nc``，Notebook 经纬验证直接读取。

官方 Iris 投影源 ``*.nc`` 仍保留在各场景目录根部（与 IMPROVER acceptance 路径一致）。

约定
----
- 历史输入场跳过 ``kgo.nc``；投影 KGO 单独写出 ``latlon/kgo.nc``
- 一日一文件；多日用例由调用方沿 ``time`` concat
- ``forecast_reference_time`` → meb ``time``；``forecast_period``（秒）→ ``dtime``（小时）
- FRT bounds → meb ``attrs['time_bounds']``；单点无 bounds 则不写
...
# 预处理派生目录名
MEB_SUBDIR = "cli_input"
LATLON_SUBDIR = "latlon"
_DERIVED_PARTS = frozenset(
    {
        MEB_SUBDIR,
        LATLON_SUBDIR,
        "cli_output",
        # 曾用名（若残留则跳过，避免当作官方源再预处理）
        "meb",
        "latlon_iris",
        "latlon_meb",
    }
...
# 官方样例中缺测常以极大浮点写入（非 _FillValue 属性）
...
def _extract_frt_and_bounds(
    ...
    if "forecast_reference_time" in da.coords:
        ...
        raise ValueError("缺少 forecast_reference_time / time 坐标。")
...
# ---------------------------------------------------------------------------
# 投影输入：投影维重命名 → cli_input
# ---------------------------------------------------------------------------


def official_to_meb6d(
    nc_path: Path, *, force_latlon_crs: bool = False
) -> xr.DataArray:
    ...
    if "height" in da.coords:
        level_coord = np.asarray(
            [float(np.asarray(da.coords["height"].values).ravel()[0])],
            dtype=np.float32,
        )
    else:
    ...
    attrs: Dict[str, Any] = {
        "units": str(da.attrs.get("units", "m s-1")),
        "dtime_units": "hour",
        "time_type": "UT",
    }
...
def build_latlon_iris_cube(src_path: Path):
    ...
    # 复制源 Cube 上非空间的标量辅助坐标（含 FRT 及其 bounds）
    skip = {
...
def save_iris_cube(cube, path: Path) -> None:
    ...
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    # 后缀必须是 .nc，否则 iris 找不到 saver；先写同目录临时名再替换
...
def iter_official_projected_sources(data_dir: Path = DATA_DIR) -> List[Path]:
    """列出官方投影源 nc（跳过 kgo 与派生目录）。"""
    files: List[Path] = []
    for nc in sorted(data_dir.rglob("*.nc")):
        if nc.name.lower() == "kgo.nc":
            continue
        if _DERIVED_PARTS.intersection(nc.parts):
            continue
        files.append(nc)
    return files


def preprocess_projected_meb(data_dir: Path = DATA_DIR) -> None:
    """投影输入：投影维重命名 → 各源目录 ``cli_input/``。"""
    sources = iter_official_projected_sources(data_dir)
    for src in sources:
        out = src.parent / MEB_SUBDIR / src.name
        save_meb6d(official_to_meb6d(src), out)
    print(f"投影输入完成：{len(sources)} 个 meb 文件。")


def iter_projection_kgo_files(data_dir: Path = DATA_DIR) -> List[Path]:
    """列出各场景投影 ``kgo.nc``（跳过已在 ``latlon/`` 下的文件）。"""
    files: List[Path] = []
    for kgo in sorted(data_dir.rglob("kgo.nc")):
        if LATLON_SUBDIR in kgo.parent.parts:
            continue
        files.append(kgo)
    return files


def preprocess_latlon_kgo(data_dir: Path = DATA_DIR) -> None:
    """投影 KGO → 同场景 ``latlon/kgo.nc``，供经纬输入 notebook 直接读取。"""
    kgos = iter_projection_kgo_files(data_dir)
    for kgo_proj in kgos:
        out = kgo_proj.parent / LATLON_SUBDIR / kgo_proj.name
        save_iris_cube(build_latlon_iris_cube(kgo_proj), out)
    print(f"经纬 KGO 完成：{len(kgos)} 个文件。")


def preprocess_latlon_branch(data_dir: Path = DATA_DIR) -> None:
    """经纬输入：投影→经纬 → ``latlon/``(Cube) + ``latlon/cli_input/``(meb)。"""
    sources = iter_official_projected_sources(data_dir)
    for src in sources:
        cube_out = src.parent / LATLON_SUBDIR / src.name
        meb_out = src.parent / LATLON_SUBDIR / MEB_SUBDIR / src.name
        cube = build_latlon_iris_cube(src)
        save_iris_cube(cube, cube_out)
        # 由刚写出的 Cube 再转 meb，保证与原方法经纬输入数值同源
        save_meb6d(official_to_meb6d(cube_out, force_latlon_crs=True), meb_out)
    print(f"经纬输入完成：{len(sources)} 组 Cube+meb 经纬文件。")


def main() -> None:
    print("=== 投影输入：投影维重命名 → cli_input/ ===")
    preprocess_projected_meb()
    print("\n=== 经纬输入：投影→经纬重网格 → latlon/ + latlon/cli_input/ ===")
    preprocess_latlon_branch()
    print("\n=== 经纬 KGO：投影 kgo.nc → latlon/kgo.nc ===")
    preprocess_latlon_kgo()
    print("\n全部预处理完成。")
