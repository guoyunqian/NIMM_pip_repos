...
def get_normalized_data_path(base_path: str) -> str:
    """Get normalized meb6d data path."""
    return os.path.join(base_path, "normalized_meb6d")
...
class TestOfficialDataValidation:
    ...
    def test_official_data_xarray_input(self):
        ...
        normalized_path = get_normalized_data_path(base_path)
        if not os.path.exists(normalized_path):
            pytest.skip(
                "normalized_meb6d data not available for strict xarray meb6d validation"
            )

        # Load normalized meb6d test data for strict xarray input check
        temp_file = os.path.join(
            normalized_path, "20181121T1200Z-PT0012H00M-temperature_at_screen_level.nc"
        )
        wind_file = os.path.join(
            normalized_path, "20181121T1200Z-PT0012H00M-wind_speed_at_10m.nc"
        )
        rh_file = os.path.join(
            normalized_path, "20181121T1200Z-PT0012H00M-relative_humidity_at_screen_level.nc"
        )
        pressure_file = os.path.join(
            normalized_path, "20181121T1200Z-PT0012H00M-pressure_at_mean_sea_level.nc"
        )
        kgo_file = os.path.join(normalized_path, "kgo.nc")

        required_files = [temp_file, wind_file, rh_file, pressure_file, kgo_file]
        missing = [f for f in required_files if not os.path.exists(f)]
        if missing:
            pytest.skip(f"normalized_meb6d files missing: {missing}")
