...
--------
1. **投影维重命名（方案一）**
   ``test_data/<数据集>/cli_input/<同名>.nc``：投影米制坐标仅改名为
   ``lat``/``lon`` → meb 六维。
...
约定
----
- 处理 ApplyGriddedLapseRate 与 LapseRate 两套算法输入
- 跳过 ``kgo.nc`` / 原方法结果 / ``cli_output/``
- 官方场多为二维投影、无 realization；地形/掩码可能无时间，meb ``time`` 占位 1970-01-01
- meb ``level`` 统一占位 0

用法（仓库根目录）::

    python temperature/cli/preprocess_test_data.py
"""
...
def official_to_meb6d(
    ...
    # 官方场高度不一致或无高度维；统一占位以免多要素 level 校验失败
    level_coord = np.array([0.0], dtype=np.float32)

    # 与 Notebook 原预处理一致：全部场使用同一占位时刻 / dtime，避免温度与掩码时效校验失败
    frt = _to_datetime64(DEFAULT_TIME)
    time_bounds = None
    dtime = 0.0
...
def iter_input_sources() -> List[Path]:
    """列出两套官方投影输入。"""
    files: List[Path] = []
    for data_dir, names in (
        (APPLY_DATA_DIR, APPLY_INPUT_FILES),
        (LAPSE_DATA_DIR, LAPSE_INPUT_FILES),
    ):
        for name in names:
            src = data_dir / name
            if not src.exists():
