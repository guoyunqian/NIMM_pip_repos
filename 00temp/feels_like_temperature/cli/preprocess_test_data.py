...
--------
1. **投影维重命名（方案一）**
   ``test_data/feels_like_temp_data/cli_input/<同名>.nc``：投影米制坐标仅改名为
   ``lat``/``lon`` → meb 六维。

2. **投影→规则经纬重网格（方案二）**
   - ``.../latlon/<同名>.nc``：经纬重网格后的 **Iris Cube**（供原 IMPROVER 方法）
   - ``.../latlon/cli_input/<同名>.nc``：同一数值的 **meb 六维**（供迁移方法）
   两边仅格式不同，场值一致。

约定
----
- 只处理四路算法输入；跳过 ``kgo.nc`` / 原方法结果 / ``cli_output/``
- ``forecast_reference_time`` → meb ``time``；``forecast_period``（秒）→ ``dtime``（小时）
- 四要素官方高度不同（1.5 m / 10 m / 海平面），meb ``level`` 统一占位 0，避免坐标校验失败
...
DATA_DIR = PACKAGE_ROOT / "test_data" / "feels_like_temp_data"

INPUT_FILES = (
    "20181121T1200Z-PT0012H00M-temperature_at_screen_level.nc",
    "20181121T1200Z-PT0012H00M-wind_speed_at_10m.nc",
    "20181121T1200Z-PT0012H00M-relative_humidity_at_screen_level.nc",
    "20181121T1200Z-PT0012H00M-pressure_at_mean_sea_level.nc",
)
...
def official_to_meb6d(
    ...
    # 气温/湿度 1.5 m、风速 10 m、气压无高度；统一占位以免四要素 level 不一致
...
def iter_input_sources(data_dir: Path = DATA_DIR) -> List[Path]:
    """列出四路官方投影输入。"""
    files: List[Path] = []
...
def preprocess_projected_meb(data_dir: Path = DATA_DIR) -> None:
    """方案一：投影维重命名 → ``cli_input/``。"""
    sources = iter_input_sources(data_dir)
    for src in sources:
        out = src.parent / "cli_input" / src.name
        save_meb6d(official_to_meb6d(src), out)
    print(f"方案一完成：{len(sources)} 个 meb 文件。")


def preprocess_latlon_branch(data_dir: Path = DATA_DIR) -> None:
    """方案二：投影→经纬 → ``latlon/``(Cube) + ``latlon/cli_input/``(meb)。"""
    sources = iter_input_sources(data_dir)
    for src in sources:
        cube_out = src.parent / "latlon" / src.name
        meb_out = src.parent / "latlon" / "cli_input" / src.name
    ...
    print(f"方案二完成：{len(sources)} 组 Cube+meb 经纬文件。")


def _verify_latlon_cube_meb_match(data_dir: Path = DATA_DIR) -> None:
    """抽查方案二 Cube 与已写出 meb 的空间场是否一致。"""
    import iris

    for src in iter_input_sources(data_dir):
        cube = iris.load_cube(str(src.parent / "latlon" / src.name))
        meb_path = src.parent / "latlon" / "cli_input" / src.name
...
def main() -> None:
    if not DATA_DIR.is_dir():
        ...
        return
    print("=== 方案一：投影维重命名 → cli_input/ ===")
    preprocess_projected_meb()
    print("\n=== 方案二：投影→经纬重网格 → latlon/ + latlon/cli_input/ ===")
