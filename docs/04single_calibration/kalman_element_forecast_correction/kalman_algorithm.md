...
```bash
python -m cli.04single_calibration.kalman_element_forecast_correction.kalman_data_main
```

运行指定日期范围：

```bash
python -m cli.04single_calibration.kalman_element_forecast_correction.kalman_data_main 20260401 20260405
```

只运行某一个变量：

```bash
python -m cli.04single_calibration.kalman_element_forecast_correction.kalman_data_main 20260401 20260401 --variables SWVL
python -m cli.04single_calibration.kalman_element_forecast_correction.kalman_data_main 20260401 20260401 --variables STL
```

复制前一天源数据：

```bash
python -m cli.04single_calibration.kalman_element_forecast_correction.trans_data_main
```

复制指定日期源数据：

```bash
python -m cli.04single_calibration.kalman_element_forecast_correction.trans_data_main 20260401
```
...
```bash
python -m cli.04single_calibration.kalman_element_forecast_correction.kalman_data_main --base-dir /data234/GUO_data/Kalman_data
python -m cli.04single_calibration.kalman_element_forecast_correction.trans_data_main --target-base /data234/GUO_data/Kalman_data/process_data
```
