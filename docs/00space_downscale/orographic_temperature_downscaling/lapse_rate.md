...
compute_lapse_rate_adjustment = _src.compute_lapse_rate_adjustment
...
| 脚本 | 对应插件 | 说明 |
| --- | --- | --- |
| `cli/00space_downscale/orographic_temperature_downscaling/anc_lapse_rate_main.py` | `ApplyGriddedLapseRate` | 应用已计算的层结递减率 |
| `cli/00space_downscale/orographic_temperature_downscaling/dsc_temp_lapse_rate_main.py` | `LapseRate` | 从温度、地形和海陆掩膜计算层结递减率 |

### 7.1 运行方式

**方式 1：直接运行示例脚本**（在脚本底部 `main()` 中修改路径与参数）

PowerShell 示例：

```powershell
python -m cli.00space_downscale.orographic_temperature_downscaling
...
result = process(
    temperature_path="00temp/orographic_temperature_downscaling/test_data/apply_lapse_rate_data/cli_input/ukvx_temperature.nc",
    lapse_rate_path="00temp/orographic_temperature_downscaling/test_data/apply_lapse_rate_data/cli_input/ukvx_lapse_rate.nc",
    source_orography_path="00temp/orographic_temperature_downscaling/test_data/apply_lapse_rate_data/cli_input/ukvx_orography.nc",
    target_orography_path="00temp/orographic_temperature_downscaling/test_data/apply_lapse_rate_data/cli_input/highres_orog.nc",
    output_path="00temp/orographic_temperature_downscaling/test_data/apply_lapse_rate_data/cli_output/cli_apply_lapse_rate_result.nc",
)
```

### 7.2 ApplyGriddedLapseRate（`anc_lapse_rate_main.py`）
...
脚本内置测试数据目录：输入 `00temp/orographic_temperature_downscaling/test_data/apply_lapse_rate_data/cli_input/`，CLI 输出 `cli_output/`。

### 7.3 LapseRate（`dsc_temp_lapse_rate_main.py`）
...
脚本内置测试数据目录：输入 `00temp/orographic_temperature_downscaling/test_data/temp_lapse_rate_data/cli_input/`，CLI 输出 `cli_output/`。

## 8. 测试数据预处理

官方投影样例需先预处理，再供 CLI / Notebook 读取。预处理脚本只保留在中间目录：

`00temp/orographic_temperature_downscaling/cli/preprocess_test_data.py`
...
写出目录（分别在 `00temp/orographic_temperature_downscaling/test_data/apply_lapse_rate_data/` 与 `temp_lapse_rate_data/` 下）：
...
- 正式目录：`15 passed, 2 skipped`（缺 `test_data/` 时官方对照 skip）
- 覆盖 `compute_lapse_rate_adjustment`、`ApplyGriddedLapseRate`、`LapseRate` 关键分支
- 官方样例对照测试从 `00temp/orographic_temperature_downscaling/test_data/` 读取
