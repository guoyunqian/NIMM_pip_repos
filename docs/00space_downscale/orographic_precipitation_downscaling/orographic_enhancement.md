...
- `NIMM/00space_downscale/orographic_precipitation_downscaling/apply_orographic_enhancement.py`
...
_src = import_module("NIMM.00space_downscale.orographic_precipitation_downscaling.orographic_enhancement")
MetaOrographicEnhancement = _src.MetaOrographicEnhancement
...
import xarray as xr
from importlib import import_module
_src = import_module("NIMM.00space_downscale.orographic_precipitation_downscaling.apply_orographic_enhancement")
ApplyOrographicEnhancement = _src.ApplyOrographicEnhancement
...
示例脚本：`cli/00space_downscale/orographic_precipitation_downscaling/dsc_orographic_enhancement_main.py`

### 7.1 运行方式

```powershell
python -m cli.00space_downscale.orographic_precipitation_downscaling
```
...
```python
from importlib import import_module
_cli = import_module("cli.00space_downscale.orographic_precipitation_downscaling.dsc_orographic_enhancement_main")
...
```powershell
python cli/00space_downscale/orographic_precipitation_downscaling/dsc_orographic_enhancement_main.py
```

内置测试数据目录：输入 `00temp/orographic_precipitation_downscaling/test_data/orographic_enhancement_data/cli_input/`，CLI 输出 `cli_output/`。
...
`00temp/orographic_precipitation_downscaling/cli/preprocess_test_data.py`

仓库根目录运行：

```text
python 00temp/orographic_precipitation_downscaling/cli/preprocess_test_data.py
```
...
| 路径 | 内容 | 用途 |
| --- | --- | --- |
| `00temp/orographic_precipitation_downscaling/test_data/orographic_enhancement_data/cli_input/` | 投影维重命名后的 meb 六维；气象场保留 `height` 作为 `level` | 投影输入验证：修改后方法 / CLI |
