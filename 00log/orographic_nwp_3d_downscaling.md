...
- 贡献人：曾晓青、丰硕、赵如奇
- 整理日期：2026-06-29
- 当前状态：已整理至 `00temp/orographic_nwp_3d_downscaling/`，尚未正式归档
...
- `00temp/` 中间目录仍保留原始 `nimm_g_interp` 包名；此前创建的正式归档目录已于 2026-08-26 撤销，当前以中间目录为准。
...
- 在 `docs/00space_downscale/orographic_nwp_3d_downscaling/figures/` 中新增 3 张 `nimm_g_interp` 输出成果图。
- 新增 FH006 原始 `g_interp` 2 米气温格点场和 `nimm_g_interp` 2 米气温格点场，用于展示插值前后空间分布。
- 新增站点 `54511`、`58362`、`59287` 的原始算法与改进算法数值对比图。
- 在算法说明文档中新增“成果展示”章节并引用上述图片。
- 本次仅更新成果图片和文档，未修改算法代码、调度程序、测试代码或测试数据。

## 2026-08-26 目录纠正

- 根据仓库当前整理流程，本算法现阶段只应更新到 `00temp/orographic_nwp_3d_downscaling/`，不应提前拆分到正式归档目录。
- 已确认 `00temp/orographic_nwp_3d_downscaling/` 保留 2026-08-17 同步后的最新算法代码、CLI、文档、资源和测试。
- 将 3 张成果图迁移到 `00temp/orographic_nwp_3d_downscaling/docs/figures/`，并在中间目录算法说明中保留“成果展示”章节。
- 撤销此前在 `NIMM/`、`cli/`、`docs/`、`nbs/`、`resource/` 和 `test/` 下创建的 g_interp 正式归档内容。
- 上文“正式归档操作”、正式目录更新和原成果图路径仅作为历史记录保留，当前归档状态以本节为准：尚未正式归档。
