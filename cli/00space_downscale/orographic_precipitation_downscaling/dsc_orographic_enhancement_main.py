...
def process(
    ...
    *,
    ...
    _src = import_module(
        "NIMM.00space_downscale.orographic_precipitation_downscaling.orographic_enhancement"
    )
...
def main() -> None:
    ...
    #各输入文件的路径映射
    temperature_path = str(cli_input_dir / "temperature.nc")   #温度场nc文件路径
    humidity_path = str(cli_input_dir / "humidity.nc")   #相对湿度场nc文件路径
    pressure_path = str(cli_input_dir / "pressure.nc")   #气压场nc文件路径
    wind_speed_path = str(cli_input_dir / "wind_speed.nc")   #风速场nc文件路径
    wind_direction_path = str(cli_input_dir / "wind_direction.nc")   #风向场nc文件路径
    orography_path = str(cli_input_dir / "orography_uk-standard_1km.nc")   #地形场nc文件路径
    ...
    if missing:
        print(
            "示例输入不存在：\n  "
            + "\n  ".join(missing)
            + "\n请补齐样例数据，或先运行 00temp/orographic_precipitation_downscaling/cli/preprocess_test_data.py，"
            "也可在此处改为自己的输入/输出路径。"
        )
    else:
        process(
